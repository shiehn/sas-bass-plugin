...
- Kick interplay: the kick (or 808) track's exact onsets are listed in the context — especially under RHYTHM ANCHORS. Land roots ON those onsets, or place deliberate pushes just off them; a bassline that drifts unrelated to the kick grid sounds detached from the band. The MAJORITY of your notes should share an onset with the kick or fall on a beat of the bar.
- Low-end space: only a LONG sustained sub (about a beat or more) can mask the kick's transient. For those notes specifically, starting just after the kick and ringing INTO the next hit is a good trick. Short and mid-length notes should land ON the kick. Never apply this as a blanket offset — a line where EVERY note sits the same distance off the kick grid is a mistake, not a groove.
